        ...
        <div
            ...
            <div
                ...
                <div className="flex-1 overflow-y-auto px-5 py-3">
                    {students.length === 0 ? <p className="text-gray-400 text-sm text-center py-4">暂无学员</p> : <ul className="divide-y divide-gray-100">
                        {students.map(s => <li key={s.id} className="flex items-center gap-2 py-2">
                            {editingId === s.id ? <>
                                <input
                                    value={editName}
                                ...
                                <button
                                    ...
                                    className="text-green-600 text-sm hover:underline">保存
                                                          </button>
                                <button
                                    onClick={() => setEditingId(null)}
                                    className="text-gray-400 text-sm hover:underline">取消
                                                          </button>
                            </> : <>
                                ...
                                <button
                                    ...
                                    className="text-blue-500 text-sm hover:underline">改名
                                                          </button>
                                <button
                                    onClick={() => {
                                    ...
                                    className="text-red-500 text-sm hover:underline">删除
                                                          </button>
                            </>}
                        </li>)}
                    </ul>}
            ...
            <div className="bg-white rounded-lg shadow-xl w-[400px]">
                ...
                <div className="px-5 py-4">
                    ...
                    <div className="flex justify-end gap-2">
                        <button
                            onClick={onClose}
                            className="px-4 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50">取消
                                        </button>
                        <button
                            onClick={() => name.trim() && onAdd(name.trim())}
                            className="px-4 py-1.5 text-sm bg-green-500 text-white rounded hover:bg-green-600">添加
                                        </button>
                    </div>
                </div>
            </div>
...
function ConfirmClearModal(
    ...
    return (
        <div
            ...
            <div className="bg-white rounded-lg shadow-xl w-[400px]">
                <div className="px-5 py-4">
                    <p className="text-sm text-gray-700 mb-4">⚠️ 确定要清空所有数据吗？此操作不可恢复。
                                  </p>
                    <div className="flex justify-end gap-2">
                        <button
                            onClick={onClose}
                        ...
                        <button
                            onClick={onConfirm}
                            className="px-4 py-1.5 text-sm bg-red-500 text-white rounded hover:bg-red-600">确认清空
                                        </button>
                    </div>
                </div>
            </div>
...
export default function ClassReportPage() {
    ...
    const updateContentSummary = (value: string) => {
        setData(prev => ({
            ...
            lessons: prev.lessons.map(l => l.id === prev.activeLessonId ? {
                ...l,
        ...
        <div className="min-h-screen bg-[#F5F7FA]">
            ...
            <div
                ...
                <button
                    ...
                    className="px-3 py-1.5 text-sm bg-red-50 text-red-500 border border-red-200 rounded hover:bg-red-100 flex items-center gap-1">清空所有数据
                            </button>
            </div>
            {}
            <div
                className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={addStudent}
                        className="px-3 py-1.5 text-sm bg-green-500 text-white rounded hover:bg-green-600">添加学员
                                  </button>
                    <button
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="px-3 py-1.5 text-sm bg-green-500 text-white rounded hover:bg-green-600">+ 增加反馈模块
                                  </button>
                </div>
                <div className="flex-1" />
                <div className="flex items-center gap-2">
                    <button
                        onClick={exportData}
                        className="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50">导出备份
                                  </button>
                    <button
                        onClick={() => fileInputRef.current?.click()}
                        className="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50">导入数据
                                  </button>
                    <input
                        ref={fileInputRef}
                        type="file"
            ...
            {}
            <div
                className="bg-white border-b border-gray-200 px-6 pt-2 flex items-end gap-1 overflow-x-auto">
                {data.lessons.map(lesson => {
                    ...
                    return (
                        <div
                            key={lesson.id}
                            className={`flex items-center gap-1 px-4 py-2 rounded-t cursor-pointer text-sm whitespace-nowrap border border-b-0 transition-colors ${isActive ? "bg-[#4A90D9] text-white border-[#4A90D9]" : "bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200"}`}
                            onClick={() => switchLesson(lesson.id)}>
                            <span
                                className="text-gray-400 text-xs mr-1 hover:text-white cursor-pointer"
                                title="上一课">‹
                                              </span>
                            <span className="font-medium">{lesson.name}</span>
                            <span
                                className="text-gray-400 text-xs ml-1 hover:text-white cursor-pointer"
                                title="下一课">›
                                              </span>
                            {data.lessons.length > 1 && <span
                                onClick={e => {
                                    e.stopPropagation();
                ...
                <button
                    ...
                    className="px-3 py-2 text-sm text-gray-500 hover:text-gray-700 border border-dashed border-gray-300 rounded-t mb-px">+ 添加课时
                            </button>
            </div>
            {}
            {currentLesson && <div className="px-6 py-4">
                {}
                ...
                {}
                {data.modules.length > 0 && <div className="bg-white rounded shadow-sm border border-gray-200 p-4 mb-4">
                    <h3 className="text-sm font-semibold text-gray-700 mb-3">反馈模块</h3>
                    <div className="flex flex-wrap gap-2">
                        {data.modules.map(mod => <span
                            key={mod.id}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th
                                        className="px-3 py-2.5 text-left font-semibold text-gray-600 w-[100px] sticky left-0 bg-gray-50 z-10">姓名
                                                            </th>
                                    <th className="px-3 py-2.5 text-center font-semibold text-gray-600 w-[140px]">基础能力反馈
                                                            </th>
                                    ...
                                    <th
                                        className="px-3 py-2.5 text-left font-semibold text-gray-600 min-w-[300px]">上课互动答题情况
                                                            </th>
                                    <th className="px-3 py-2.5 text-center font-semibold text-gray-600 w-[80px]">操作</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.students.length === 0 ? <tr>
                                    <td colSpan={8} className="px-4 py-8 text-center text-gray-400">暂无学员，请在上方添加学员
                                                              </td>
                                </tr> : data.students.map(student => {
                                    ...
                                    return (
                                        <tr key={student.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                                            <td
                                                className="px-3 py-2 font-medium text-gray-800 sticky left-0 bg-white z-10">
                                                {student.name}
                                            </td>
                                            ...
                                            <td className="px-3 py-2 text-center">
                                                <button
                                                    ...
                                                    className="px-3 py-1 text-sm bg-green-500 text-white rounded hover:bg-green-600">复制
                                                                                </button>
                                            </td>
                                        </tr>
                                    );
